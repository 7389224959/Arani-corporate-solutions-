...
const TemplateEditor: React.FC<TemplateEditorProps> = ({ template: initialTemplate, onSave, onCancel }) => {
  ...
  const handleDragStart = (e: React.MouseEvent | React.TouchEvent, boxName: string, action: 'move' | 'resize') => {
    e.preventDefault();
    e.stopPropagation();
    ...
    setStartCoords(parseCoords((template.coordinates as any)?.[boxName] || 'hidden'));
  };
          ...
          <div>
             ...
             <div className="grid grid-cols-2 gap-4">
               <div>
                  ...
                  <input type="number" 
                    value={template.safe_limits?.headline_words || 10} 
                    onChange={e => setTemplate({...template, safe_limits: {...(template.safe_limits || { subtitle_lines: 3, ticker_characters: 100 }), headline_words: parseInt(e.target.value) || 10}})}
               ...
               <div>
                  ...
                  <input type="number" 
                    value={template.safe_limits?.subtitle_lines || 3} 
                    onChange={e => setTemplate({...template, safe_limits: {...(template.safe_limits || { headline_words: 10, ticker_characters: 100 }), subtitle_lines: parseInt(e.target.value) || 3}})}
                    className="w-full p-2 border rounded outline-none"
                  />
               </div>
             </div>
